...
1. Collect all the windows that match chosen criteria to any workspace you want, scatter them across multiple workspaces, or even tile them so that they fit on a chosen screen.
2. Run custom commands on windows matching criteria, such as maximizing them, changing their opacity, or closing them.
3. Dynamically add, remove, or toggle niri configuration rules on the fly without needing to edit anything manually.
...
- `make.sh`: Builds niri (and xwayland-satellite) from source in a docker container
  - Supports `clean` (to invalidate the docker build cache) and `install`
- `niriu.sh`: Script helper for niri window management

## Try it Out

If you need to build niri from source and/or learn how to install the script head down to the [Installation](#installation) section below, otherwise just make the script executable and copy it to somewhere in your PATH before you continue.
...
The following assumes you have the repository cloned locally and are running a shell session in the repository root directory.
...
To install niri (and xwayland-satellite) from source using the provided Dockerfile and build script, run:

```sh
./make.sh install
```

The script will ask for your password to install the built niri binary to `/usr/local/bin/`. If you want to just build niri without installing, simply run `./make.sh` without the `install` argument and the built binaries will be copied to the `./package/` directory.

To install the `niriu.sh` simply copy `niriu.sh` to a directory in your PATH and make it executable, e.g.:

```sh
chmod +x ./niriu.sh
cp ./niriu.sh ~/.local/bin/
```
